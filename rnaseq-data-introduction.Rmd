...
Please download your favorite Synapse client: [R](https://r-docs.synapse.org/) or [Python client](https://python-docs.synapse.org//build/html/index.html). Refer to the necessary docs [synapser docs](https://r-docs.synapse.org/articles/synapser.html#connecting-to-synapse) or [Python](https://python-docs.synapse.org//build/html/index.html) to successfully log in to synapse and start using its various analytical tools.
...
* Look up the SynID of the required file and then use the __synGet__ command to access the folder. 
* Download the required csv file by adding __$path__ to the accessed file
...
fileEntity <- synGet('syn18137070')
# Download the data file
genes.with.meta<-read.csv(gzfile(fileEntity$path))
```
...
* The Structure of the file refers the column names and the type of variables stored in it:
  * _id_ == the synapseIDs of the individual files with raw data
  * _specimenID_ == ID of individual samples
...
```{r explore_data, echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
...
```{r explore_metadata, echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
...
```{r Gene_exp_lookup, echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
...
To explore the data and find meaningful differences between samples, we first perform some dimensionality reduction using principal component analysis. We can then look at the genes that are top contributors to the first few principal components. Here we notice that the dataset seems to have two distinct clusters. We encourage the participant to investigate what may be contributing to this clustering (_hint: color the plot by different column names to findout_)

```{r PCA, echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE}

###PCA plots  

doPcaPlots<-function(datafile, env){
  #create matrix
  combined.mat=reshape2::acast(datafile,Symbol~id,value.var="zScore")
  ...
  env$tab1 <- ggplot(subset(datafile,Symbol%in%genes1[1:10]))+
  ...
  env$tab2 <- ggplot(subset(datafile,Symbol%in%genes2[1:10]))+
              geom_jitter(aes(x=Symbol,y=zScore,col=tumorType,shape=study))+
...
Here we notice that the dataset seems to have two distinct clusters. We encourage the participant to investigate what may be contributing to this clustering (_hint: color the plot by different column names to findout_)
...
```{r DEG_analysis, echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
...
```{r GOsummaries, echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
...
```{r PathwayAnalysis, echo=FALSE, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
